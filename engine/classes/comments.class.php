...
class DLE_Comments {
	...
	function compile_comment( $tpl, $row, $area, $indent ) {
		...
		if( isset($row['uploaded_images']) AND is_array($row['uploaded_images']) AND count($row['uploaded_images']) ) {
			...
			foreach ($row['uploaded_images'] as $temp_value) {

				$path_parts = get_uploaded_image_info(trim($temp_value));
				
				if($path_parts->thumb) {
					
					$gallery_image[] = "<li><a href=\"{$path_parts->url}\" data-highslide=\"comments_image_{$row['id']}\" target=\"_blank\"><img src=\"{$path_parts->thumb}\" alt=\"\"></a></li>";
				...
				$tpl->set( '{images}', "<ul class=\"comments-image-gallery\">".implode($gallery_image)."</ul>" );
	...
	function build_customcomments( $tpl, $template ) {
		...
		if(is_array($display_images) AND count($display_images) ) {
			...
			$sql_result = $this->db->query( "SELECT c_id, name FROM " . PREFIX . "_comments_files WHERE c_id IN ({$display_images})" );
			
			while ( $row = $this->db->get_row( $sql_result ) ) {
				if( is_array( $rows[$row['c_id']] ) ) $rows[$row['c_id']]['uploaded_images'][] = $row['name'];
			}
