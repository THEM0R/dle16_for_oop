...
function custom_comments( $matches=array() ) {
	...
	if( $content !== false ) {
		...
		if ( $config['allow_cache'] ) create_cache( "news", $content, $custom_cache_id, true );
...
function news_sort($do) {
	...
	if( isset( $_SESSION[$direction_sort] ) AND ($_SESSION[$direction_sort] == "desc" OR $_SESSION[$direction_sort] == "asc") )
    {

      $direction = $_SESSION[$direction_sort];
    }
